...
import { Edit, MoreHorizontal, DollarSign, CheckCircle } from "lucide-react"
...
export function PayrollTable({ entries, isLoading, onRefresh, periodType, startDate, endDate }: PayrollTableProps) {
  ...
  const handleMarkAllAsPaid = () => {
    const unpaidEntries = entries.filter((entry) => entry.status === "unpaid").map((entry) => entry.id)
    ...
    <>
      <div className="rounded-md border">
        <div className="flex justify-end p-4">
          <Button onClick={handleMarkAllAsPaid} disabled={entries.every((entry) => entry.status === "paid")}>
            ...
            {entries.length === 0 ? (
              ...
              entries.map((entry) => (
                <TableRow key={entry.id}>
                  ...
                  <TableCell>
                    {entry.regular_hours.toFixed(1)} + {entry.overtime_hours.toFixed(1)} OT
                  </TableCell>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Update the payroll entry details below.</DialogDescription>
          </DialogHeader>
          {selectedEntry && (
            <PayrollEntryForm
              entry={selectedEntry}
      ...
      <AlertDialog open={isPayDialogOpen} onOpenChange={setIsPayDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will mark {selectedEntries.length > 1 ? "these payroll entries" : "this payroll entry"} as paid with
              today's date.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={confirmMarkAsPaid} disabled={isProcessing}>
